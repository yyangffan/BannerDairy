...
public class BlackEditText extends EditText {
	...
	public BlackEditText(Context context, AttributeSet attrs) {
		...
		mRectAll=new Paint();
		mRectAll.setStyle(Paint.Style.STROKE);
		mRectAll.setColor(Color.BLACK);
		mRectAll.setAntiAlias(true);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < PWD_LENGTH; i++) {
			...
			mRect = new Rect(left, top-5, right, bottom-5);
			canvas.drawRect(mRect, mRectPaint);

		}
		canvas.drawRoundRect(new RectF(0, 0, mWidth, mHeight-10),7,7,mRectAll);
		// 绘制密码
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		super.onMeasure(widthMeasureSpec, (int) getResources().getDimension(R.dimen.y80));
	}
}
