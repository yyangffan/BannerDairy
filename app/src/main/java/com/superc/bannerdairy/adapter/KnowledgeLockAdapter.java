...
import com.superc.bannerdairy.databinding.ItemKnowledgeBinding;
...
public class KnowledgeLockAdapter extends RecyclerView.Adapter  {
    protected List<KnowledgeItem> mDataList;// 数据列表
     ...
     */
    public KnowledgeLockAdapter(Context context, List<KnowledgeItem> dataList, int layoutId, int variableId) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        ItemKnowledgeBinding itemBinding = (ItemKnowledgeBinding) binding;

        binding.setVariable(variableId, mDataList.get(position));
        if(mDataList.get(position).getArticle_image()!=null) {
            Glide.with(mContext).load(ConnectUrl.REQUESTURL +mDataList.get(position).getArticle_image()).into(itemBinding.image);
        }
